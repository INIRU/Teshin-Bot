...
function CreateText(text) {
  ...
  for (const key in listData) {
    for (const i in data[listData[key]]) {
...
module.exports = {
  ...
  execute(client) {
    setInterval(function () {
      const channel = client.channels.cache.get('961881095727620106');
      fs.readFile('./src/database/teshin.json', 'utf-8', async (err, data) => {
        ...
        const text = [];
        for (const key in SliceTeshin) {
          let temp = '';
          for (const value in SliceTeshin[key]) {
